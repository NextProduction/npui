...
export function FeaturedComponents() {
  const featured = npuiComponents.slice(0, 3) // Take first 3 as featured
  ...
  return (
    <section className="py-16 md:py-24 bg-npui-background">
      <div className="container px-4 md:px-6">
        ...
        >
          <h2 className="featured-heading text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl text-center mb-12 opacity-0 translate-y-10">
            Explore Our Top Components
          </h2>
        </GSAPAnimationWrapper>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {featured.map((component, index) => (
            <GSAPAnimationWrapper
              key={component.slug}
              selector={`.component-card-${index}`}
              animationProps={{ opacity: 1, y: 0, duration: 1, delay: 0.1 * index, ease: "power3.out" }}
              scrollTriggerProps={{
                trigger: `.component-card-${index}`,
                start: "top 90%",
                toggleActions: "play none none none",
              }}
            >
              <div className={`component-card-${index} opacity-0 translate-y-10`}>
